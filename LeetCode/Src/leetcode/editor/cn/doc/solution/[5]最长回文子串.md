...
\begin{equation*}
    ...
    true &\text {如果子串 }S_{i},\cdots,S_{j}\text{是回文串} \\
    false &\text {其他情况 }
    \end{array}\right.
    \end{equation*}
$$
...
print(longestPalindrome('abcdefgfedcba'))
```
...
public:
	string longestPalindrome(string s)
	{
		...
		for (int i = 0; i < s.size(); ++i)
		{
			...
			while (right < s.size() - 1 && s[left] == s[right + 1]) right++;
...
int main()
{
	...
	cout << solution.longestPalindrome("abcdefgfedcba") << endl;
}
```
