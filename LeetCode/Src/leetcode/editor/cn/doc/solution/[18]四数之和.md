...
public:
	vector<vector<int>> fourSum(vector<int>& nums, int target) {
		...
		int a = 0, d = nums.size() - 1;

		while (a < d)
		{
			int b = a + 1, c = d - 1;
			int sum0 = nums[a] + nums[b] + nums[c] + nums[d];

			while (b < c)
			{
				int sum4 = nums[a] + nums[b] + nums[c] + nums[d];
				if (sum4 == target) results.push_back({ nums[a], nums[b], nums[c], nums[d] });
				if (sum4 <= target) ++b;
				if (sum4 >= target)--c;
			}
...
def four_sum(nums: List[int], target: int) -> List[List[int]]:
    ...
    while a < d:
        b, c = a + 1, d - 1
        sum0 = nums[a] + nums[b] + nums[c] + nums[d]
