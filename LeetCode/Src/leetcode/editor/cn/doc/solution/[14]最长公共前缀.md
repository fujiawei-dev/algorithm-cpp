...
public:
	string longestCommonPrefix(vector<string>& strs) {
		...
		int end = 0, length = strs[0].size(), count = strs.size();
		while (end < length)
		{
			char c = strs[0][end];
			for (int i = 1; i < count; ++i)
			{
				if (end >= strs[i].size() || c != strs[i][end])
				{
...
def longest_common_prefix(strings: List[str]):
    ...
    end, length, count = 0, len(strings[0]), len(strings)
    while end < length:
        c = strings[0][end]
        for i in range(1, count):
            if end >= len(strings[i]) or c != strings[i][end]:
